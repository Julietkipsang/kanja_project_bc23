page 50141 "Merchant Commision acc List"
{
    // version TL2.0
    ...
    SourceTableView = where(Status = filter(Active), "Vendor Type" = filter('Merchant Account'), Commision = const(true));
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                field(FintechAccount; Rec.FintechAccount)
                {
                    Editable = false;
                    ApplicationArea = All;
                }
