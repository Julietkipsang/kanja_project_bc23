page 50040 "Merchants Account List"
{
    // version TL2.0
    ...
    SourceTableView = where(Status = filter(Active), "Vendor Type" = filter('Merchant Account'), Commision = const(false));
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                field(AgentAccount; Rec.AgentAccount)
                {
                    Editable = false;
                    ApplicationArea = All;
